...
if __name__ == "__main__":
    ...
    [client, bot_e, bot_a] = Actions.bot_init(False)
    ...
    if len(point) >= 2:
        lx = int(point[0] - bot_e.client_rect[0])
        ly = int(point[1] - bot_e.client_rect[1])
        temp_client = copy.deepcopy(bot_e.curr_client)
        temp_client = cv2.circle(temp_client, (lx, ly), 10, color=(0, 0, 255), thickness=3)
