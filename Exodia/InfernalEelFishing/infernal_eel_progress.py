"""
Inventory progress score for infernal eel fishing.

Tracks infernal eel slot labels over time:

- **Increase** — fishing progress (more eel slots) → score up
- **Decrease** — cracking progress (eels turned into shards/tips) → score up
- **No change** — stagnation; streak counter rises, no score gain
"""
...
class InventoryProgressScore:
    """Tracks eel-labeled slot changes as a session health metric."""
    ...
    def observe(self, eel_count: int, inv_slots: Optional[int] = None) -> str:
        """
        Record eel slot count and occupied inv slots.

        Returns ``baseline``, ``fishing``, ``cracking``, ``inv_change``, or ``stagnant``.
        """
